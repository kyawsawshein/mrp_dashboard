...
export function calculateMultiLevelMRP(
  ...
  inventoryItems.forEach(item => {
    ...
    mrpResults.set(item.id, {
      ...
      reserved: (item as any).allocatedStock || 0,
      available: item.currentStock - ((item as any).allocatedStock || 0),
      safetyStock: (item as any).safetyStock || 0,
  ...
  const activeOrders = orders.filter(order => 
    order.status === 'planned' || order.status === 'in-progress' || (order.status as string) === 'released'
  );
  
  activeOrders.forEach(order => {
    // Find product (exact match, then partial word overlap match)
    const orderWords = order.product.toLowerCase().split(/\s+/).filter(w => w.length > 3);
    const product = products.find(p => p.name === order.product) ||
      products.find(p => {
        const productWords = p.name.toLowerCase().split(/\s+/);
        const matchCount = orderWords.filter(w => productWords.some(pw => pw.includes(w) || w.includes(pw))).length;
        return matchCount >= Math.min(2, orderWords.length);
      });
    if (!product || !product.bomId) {
      console.warn(`Product or BOM not found for order ${order.id}`);
      return;
    }
